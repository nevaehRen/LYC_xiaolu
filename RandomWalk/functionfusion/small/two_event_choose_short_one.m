...
Happens = Happens(randperm(sum(size(Happens))-1));    

First  = Happens(1:2:end);
Second = Happens(2:2:end);


Length = sum(size(Second))-1;
First  = First(1:Length);
Second = Second(1:Length);


Docking_High=min([First';Second';])';
...
plot((X),(Y),'.','MarkerSize',10,'Color','r')%Colors(1,:))
